...
local oconfig = {
    ...
    false, -- 55 Совсем заебали 1
    false, -- 56 2
    false, -- 57 3
    2, -- 58 Zaebali threshold
    '' -- 59 КМД офф Бота 2! 
}
-- Паттерны в ключах
patternon = true
...
local mm_state = imgui.ImBool(false)
...
local small_buffer1 = imgui.ImBuffer(48)
local small_buffer2 = imgui.ImBuffer(48)
local small_buffer3 = imgui.ImBuffer(48)
local small_buffer4 = imgui.ImBuffer(48)
local small_buffer5 = imgui.ImBuffer(64)
local small_buffer6 = imgui.ImBuffer(64)
local small_buffer7 = imgui.ImBuffer(64)
local small_buffer8 = imgui.ImBuffer(48)
local small_buffer9 = imgui.ImBuffer(48)
local small_buffer10 = imgui.ImBuffer(48)
local small_buffer11 = imgui.ImBuffer(48)
local small_buffer12 = imgui.ImBuffer(48)
local small_buffer13 = imgui.ImBuffer(48)
...
-- main
local su_state = imgui.ImBool(false)
function main()
    ...
    script_message('IAH Reloaded 1.0 (03-12-2020) by mhertz загружен!')
    script_message('Команды: tiah (Вкл/Выкл), iah (Меню)')
    ...
    while true do
        wait(0)
        imgui.Process = main_window_state.v or bar_state.v or mm_state.v or su_state.v
        if enabled then
            if zaebali >= config[58] then
                crash_flag = true
            end
            oldpos[1] = newpos[1]
            oldpos[2] = newpos[2]
            ...
            elseif oldpos[1] - newpos[1] > 300.0 or oldpos[1] - newpos[1] < -300.0 or oldpos[2] - newpos[2] > 300.0 or oldpos[2] - newpos[2] < -300.0 then
                ...
                if config[51] == true then
                    if config[49] ~= '' then sampProcessChatInput(config[49]) end
                    if config[59] ~= '' then sampProcessChatInput(config[59]) end
                end
...
function paratotext_preprocess(a)
    a = a:gsub(',%s+',',')
    a = split(a, ',')
...
function paratotext(a, b, s)
    ...
    gbuffer1 = a
    gbuffer2 = b
    s = s:gsub('@random%((.+)%)',paratotext_preprocess)
...
-- Функция ответа
function printText(text, dialog, vtype, noexec, doreset)
...
function processText(text, dialog)
    ...
    elseif text:find(config[9], 1, patternon) ~= nil and text:find(config[10], 1, patternon) ~= nil and config[32] == true then
        ...
        if config[55] == true then
            zaebali = zaebali + 1
        end
        return printText(config[35][math.random(#config[35])], dialog,verboseType.MISMATCH_1, false, false)
    elseif text:find(config[11], 1, patternon) ~= nil and text:find(config[12], 1, patternon) ~= nil and config[33] == true then
        ...
        patternnotknow = patternnotknow + 1
        if config[56] == true then
            zaebali = zaebali + 1
        end
        return printText(config[35][math.random(#config[35])], dialog,verboseType.MISMATCH_2, false, false)
    elseif text:find(config[19], 1, patternon) ~= nil and text:find(config[20], 1, patternon) ~= nil and config[34] == true then
        ...
        patternnotknow = patternnotknow + 1
        if config[57] == true then
            zaebali = zaebali + 1
        end
        return printText(config[35][math.random(#config[35])], dialog,verboseType.MISMATCH_3, false, false)
...
-- Packet Manager
abc = imgui.ImInt(0)
def = imgui.ImBuffer(64)
ghi = imgui.ImBool(false)
data = {}
cyrlower3 = {'А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ё', 'Ж', 'З', 'И', 'Й', 'К', 'Л', 'М', 'Н', 'О', 'П', 'Р', 'С', 'Т', 'У', 'Ф', 'Х', 'Ц', 'Ч', 'Ш', 'Щ', 'Ь', 'Ы', 'Ъ', 'Э', 'Ю', 'Я'}
cyrlower4 = {'а', 'б', 'в', 'г', 'д', 'е', 'е', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'о', 'п', 'р', 'с', 'т', 'у', 'ф', 'х', 'ц', 'ч', 'ш', 'щ', 'ь', 'ы', 'ъ', 'э', 'ю', 'я'}
function lower_cyr2(text)
    if text == nil then
        return
    end
    text = string.lower(text)
    for i, y in ipairs(cyrlower3) do
        text = text:gsub(y,cyrlower4[i])
    end
    return text
end
-- ImGui
...
function imgui.OnDrawFrame()
    ...
    if mm_state.v then
        ...
        imgui.SetNextWindowSize(imgui.ImVec2(400, 220), imgui.Cond.FirstUseEver)
        imgui.Begin(u8'IAH Menu (1.0)', mm_state)
        imgui.SetCursorPosX(85)
        if imgui.Button(u8'Настройки', imgui.ImVec2(230, 50)) then
            main_window_state.v = true
            mm_state.v = false
        end
        imgui.SetCursorPosX(85)
        if imgui.Button(u8'Диспетчер дополнений', imgui.ImVec2(230, 50)) then
            mm_state.v = false
            su_state.v = true
        end
        imgui.SetCursorPosX(85)
        if imgui.Button(u8'ВКЛ/ВЫКЛ скрипт', imgui.ImVec2(230, 50)) then
            tiah()
        end
    ...
    if main_window_state.v then
        ...
        local imzaebali1 = imgui.ImBool(config[55])
        local imzaebali2 = imgui.ImBool(config[56])
        local imzaebali3 = imgui.ImBool(config[57])
        local imzaebali = imgui.ImInt(config[58])
        ...
        small_buffer13.v = u8:encode(config[59])
        ...
        imgui.Begin(u8'Настройки IAH (ver 1.0)', main_window_state, imgui.WindowFlags.MenuBar)
        ...
        if imgui.CollapsingHeader(u8"Настройки ключевых слов") then
            ...
            imgui.SameLine(395)
            imgui.Text(u8'Настройки "Заебали":')
            imgui.BeginChild("##ecmascript", imgui.ImVec2(375, 115), true, imgui.WindowFlags.NoScrollbar)
            ...
            imgui.EndChild()
            imgui.SameLine()
            imgui.BeginChild("##ecmascript2", imgui.ImVec2(375, 115), true, imgui.WindowFlags.NoScrollbar)
            if imgui.Checkbox(u8'1-я группа слов', imzaebali1) then
                config[55] = imzaebali1.v
            end
            if imgui.Checkbox(u8'2-я группа слов', imzaebali2) then
                config[56] = imzaebali2.v
            end
            if imgui.Checkbox(u8'3-я группа слов', imzaebali3) then
                config[57] = imzaebali3.v
            end
            imgui.EndChild()
        end
        ...
        if imgui.CollapsingHeader(u8"Настройки обычных фраз и параметров") then
            ...
            if imgui.InputText(u8'Команда бота #2', small_buffer13) then
                config[59] = u8:decode(small_buffer13.v)
            end
            if imgui.SliderInt(u8'Порог "Заебали"', imzaebali, 1, 10) then
                config[58] = imzaebali.v
            end
            imgui.EndChild()
        end
    ...
    if su_state.v then
        ...
        imgui.Begin(u8'Диспетчер дополнений', su_state, imgui.WindowFlags.MenuBar)
        if imgui.BeginMenuBar() then
            if imgui.MenuItem(u8'Главное меню') then
                mm_state.v = true
                su_state.v = false
            ...
            if imgui.Button(u8'Загрузить') then
                if not supportOnlineFeatures then script_message('У вас версия без поддержки Online-фич.')
                else
                    local body = https.request("https://raw.githubusercontent.com/xefinity/iah2/main/addons/masterfile.json")
            ...
            imgui.InputText(u8'Поиск', def)
            imgui.PopItemWidth()
            imgui.Separator()
            imgui.PushItemWidth(150)
            imgui.Combo("##aaaaaa",abc,{u8'Всё',u8'Вопросы',u8'Конфиги',u8'Плагины',u8'Другое'})
            imgui.PopItemWidth()
            imgui.Separator()
            imgui.PushItemWidth(150)
            imgui.Checkbox(u8'Legacy',ghi)
            imgui.PopItemWidth()
            imgui.EndMenuBar()
        end
        if type(data) == 'table' and #data ~= 0 then
            for i,j in ipairs(data) do
                if j.minver <= IAH2_version_id
                and (j.maxver == -1 or j.maxver >= IAH2_version_id)
                and ((j.legacy == true and ghi.v == true) or j.legacy == false)
                and string.find(lower_cyr2(j.name), lower_cyr2(u8:decode(def.v)), 1, true) ~= nil
                and (abc.v == 0 or abc.v-1 == j.type) then
                    imgui.BeginChild("##element" .. tostring(i), imgui.ImVec2(750, 250), true, imgui.WindowFlags.NoScrollbar)
                    imgui.Text(u8:encode(j.name))
                    imgui.Separator()
                    imgui.Columns(2)
                    imgui.Text(u8:encode('Автор: ' .. j.author))
                    imgui.Text(u8:encode('Мин. версия: ') .. j.minver)
                    imgui.Text(u8:encode('Макс. версия: ' .. (j.maxver == -1 and 'Любая' or j.maxver)))
                    imgui.NextColumn()
                    imgui.Text(u8:encode('Дата: ') .. j.date)
                    if j.type == 0 then
                        imgui.Text(u8:encode('Тип: Вопросы'))
                    elseif j.type == 1 then
                        imgui.Text(u8:encode('Тип: Конфиг'))
                    elseif j.type == 2 then
                        imgui.Text(u8:encode('Тип: Плагин'))
                    else
                        imgui.Text(u8:encode('Тип: Другое'))
                    end
                    imgui.Text(u8:encode('Легаси: ' .. (j.legacy and 'Да' or 'Нет')))
                    imgui.Columns(1)
                    imgui.Spacing()
                    imgui.Separator()
                    ...
                    if imgui.Button(u8'Установить') then
                        if j.ctype == false then
                            loadstring(j.cstring)()
                            printStringNow('~g~Installed',2000)
                            addOneOffSound(0.0, 0.0, 0.0, 1138)
                        else
                            if doesFileExist(getWorkingDirectory() .. '\\' .. j.cname) then os.remove(getWorkingDirectory() .. '\\' .. j.cname) end
                            downloadUrlToFile(j.cstring, getWorkingDirectory() .. '\\' .. j.cname, function(id, status, p1, p2)
                                if status == dlstatus.STATUS_DOWNLOADINGDATA then
                                    print(string.format('Загружено %d из %d.', p1, p2))
                                elseif status == dlstatus.STATUS_ENDDOWNLOADDATA then
                                    script_message('Пакет установлен. Перезагрузка скриптов...')
                                    lua_thread.create(function() wait(500) reloadScripts() end)
                                end
                                if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                                    script_message('Не удалось установить пакет.')
                                end
                            end)
                        end
                    end
